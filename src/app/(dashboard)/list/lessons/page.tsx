...
function renderRow(role: string | undefined) {
  return function LessonRow(item: LessonList) {
    return (
      <tr
        key={item.id}
...
Object.assign(renderRow, { displayName: "LessonRowRenderer" });
...
const LessonListPage = async ( {searchParams} : { searchParams: {[key: string]:string | undefined }}) => {
  ...
  return (
    <div className="bg-white p-4 m-4 mt-0 rounded-md flex-1">
      ...
      {/* List */}
      <Table columns={[...baseColumns, ...(role === "admin" ? [{ header: "Actions", accessor: "action" }] : [])]} renderRow={renderRow(role)} data={data} />
